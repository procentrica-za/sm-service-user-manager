...
func (s *Server) handlegetverificationstatus() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		//get userID from crud service
		req, respErr := http.Get("http://" + config.CRUDHost + ":" + config.CRUDPort + "/status?userid=" + userid)

		//check for response error of 500
		if respErr != nil {
			w.WriteHeader(500)
			fmt.Fprint(w, respErr.Error())
			fmt.Println("Error in communication with CRUD service endpoint for request to retrieve status")
			return
		}
		if req.StatusCode != 200 {
			w.WriteHeader(500)
		...
		if req.StatusCode == 500 {
			...
			bodyString := string(bodyBytes)
			fmt.Fprintf(w, "An internal error has occured whilst trying to get status"+bodyString)
			fmt.Println("An internal error has occured whilst trying to get status" + bodyString)
			return
		}
		...
		//create new response struct
		var getResponse Status
		decoder := json.NewDecoder(req.Body)
		err := decoder.Decode(&getResponse)
		if err != nil {
			w.WriteHeader(500)
			fmt.Fprint(w, err.Error())
			fmt.Println("An internal error has occured whilst trying to decode the get outstanding status")
			return
		}
		...
		if jserr != nil {
			w.WriteHeader(500)
			fmt.Fprint(w, jserr.Error())
			fmt.Println("Error occured when trying to marshal the response to get status")
			return
		}
...
func (s *Server) handlepurchaseads() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Println("Handle Purchase ads has been called...")
		...
		//create byte array from JSON payload
		requestByte, _ := json.Marshal(purchaseAdvertisementToken)

		//put to crud service
		req, err := http.NewRequest("PUT", "http://"+config.CRUDHost+":"+config.CRUDPort+"/purchase", bytes.NewBuffer(requestByte))
		if err != nil {
			fmt.Fprint(w, err.Error())
			fmt.Println("Error in communication with CRUD service endpoint for request to purchase advertisement token")
			return
		}
		...
		//create new response struct
		var purchaseAdvertisementResult PurchaseAdvertisementResult

		decoder := json.NewDecoder(resp.Body)
		err = decoder.Decode(&purchaseAdvertisementResult)
		if err != nil {
			w.WriteHeader(500)
			fmt.Fprint(w, err.Error())
			return
		...
		//convert struct back to JSON
		js, jserr := json.Marshal(purchaseAdvertisementResult)
		if jserr != nil {
			w.WriteHeader(500)
			fmt.Fprint(w, jserr.Error())
			fmt.Println("Error occured when trying to marshal the response to update user")
